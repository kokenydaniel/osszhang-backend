...
class AIFinanceService
{
    ...
    public function overspendRootCause(Household $household, array $validated): array
    {
    ...
    public function cashflowForecast(Household $household, array $validated): array
    {
        $this->ensureHousehold($household);
        $monthPrefix = sprintf('%04d-%02d', $validated['year'], $validated['month']);
        $transactions = Transaction::where('household_id', $household->id)
    ...
    public function savingsRecommendations(Household $household, array $validated): array
    {
        $this->ensureHousehold($household);
    ...
    public function optimizeDebts(Household $household, string $strategy): array
    {
        $this->ensureHousehold($household);
    ...
    public function weeklyBriefing(Household $household, ?string $weekStart): array
    {
        $this->ensureHousehold($household);
    ...
    public function query(User $user, string $prompt, bool $includeContext): array
    {
        $context = [];
        if ($includeContext) {
            $household = $user->household;

            $transactions = Transaction::where('household_id', $user->household_id)->with('items')->get();
            if ($household) {
                $this->cipher->ensureCipherKey($household);
            }
            $income = (float) $transactions
            ...
            $savings = Saving::where('household_id', $user->household_id)->with('ledger')->get();
            $savingsTotal = (float) $savings->sum(function ($s) use ($household) {
                return $s->ledger->sum(fn ($e) => (float) ($this->crypto->ledgerResolved($e, $household)['amount'] ?? 0));
            });

            $debts = Debt::where('household_id', $user->household_id)->get();
